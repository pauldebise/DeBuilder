...
| **Tableau de bord (zone principale)** | Badge d'état de la boucle en haut de page (active avec itération en cours / arrêt en cours / mission terminée / arrêtée / interrompue — via heartbeat `~/.debuilder/heartbeat.json` + PID `~/.debuilder/loop_pid.txt`), barre de progression de la mission (ligne de bilan `Couverture : X / N` de `SPEC_COVERAGE.md`, avec repli sur le comptage des lignes du tableau), résumé en langage naturel de l'activité en cours (généré par LLM ou par heuristique de repli), avancement (`PROGRESS.md` parsé), alertes watchdog/système (y compris l'état du circuit breaker API), et flux des logs OpenCode quasi temps réel (Server-Sent Events sur `/api/logs/stream`, style terminal). |
